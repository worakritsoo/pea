/* icon.svelte generated by Svelte v3.29.4 */
"use strict";

const { SvelteComponent, action_destroyer, append, assign, binding_callbacks, compute_rest_props, create_slot, detach, element, exclude_internal_props, get_spread_update, init, insert, is_function, safe_not_equal, set_attributes, set_data, space, text, transition_in, transition_out, update_slot } = require("svelte/internal");
...
function instance($$self, $$props, $$invalidate) {
	...
	$$self.$$.update = () => {
		if ($$self.$$.dirty & /*theme, ios, md, aurora*/ 552960) {
			$: if (theme) {
				if (theme.ios) $$invalidate(21, themeIcon = ios);
				if (theme.md) $$invalidate(21, themeIcon = md);
				if (theme.aurora) $$invalidate(21, themeIcon = aurora);
			}
		}

		if ($$self.$$.dirty & /*themeIcon, material, f7, icon*/ 2100736) {
			$: if (themeIcon) {
				...
				if (prop === "material" || prop === "f7") {
					$$invalidate(20, classes["material-icons"] = prop === "material", classes);
					$$invalidate(20, classes["f7-icons"] = prop === "f7", classes);
				}

				if (prop === "icon") {
					$$invalidate(20, classes[value] = true, classes);
				}
			} else {
				$$invalidate(20, classes = {
					icon: true,
					"material-icons": material,
					"f7-icons": f7
				});

				if (icon) $$invalidate(20, classes[icon] = true, classes);
			}
		}

		$: $$invalidate(3, iconClasses = classNames(className, classes, colorClasses($$props)));

		if ($$self.$$.dirty & /*theme*/ 524288) {
			$: $$invalidate(4, iconText = iconTextComputed(theme));
		}

		if ($$self.$$.dirty & /*size*/ 32768) {
			$: $$invalidate(22, iconSize = typeof size === "number" || parseFloat(size) === size * 1
			? `${size}px`
			: size);
		}

		if ($$self.$$.dirty & /*style, iconSize*/ 4194432) {
